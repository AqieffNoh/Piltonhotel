...
if(isset($_POST["type"]) && $_POST["type"]=='add')
{
	...
	$results = $conn->query("SELECT merch_name, merch_id, price FROM merch WHERE merch_id ='$Product_ID' LIMIT 1");
	$obj = $results->fetch_object();
	
	if ($results) { //we have the product info 
		
		//prepare array for the session variable
		$new_product = array(array('name'=>$obj->merch_name, 'code'=>$Product_ID , 'TiradaProductTiga'=>$product_qty, 'Qiimaha'=>$obj->price));
